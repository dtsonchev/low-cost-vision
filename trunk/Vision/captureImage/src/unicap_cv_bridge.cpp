...
{
	if(!SUCCESS(unicap_set_property_value(handle, "White Balance Blue", blue)))
	{
		throw unicap_cv_exception("Failed to set property \"White Balance Blue\"");
	}

	if(!SUCCESS(unicap_set_property_value(handle, "White Balance Red", red)))
	{
...
void unicap_cv_camera::set_auto_white_balance(bool automatic)
{
	if(automatic)
	{
		if (!SUCCESS(unicap_set_property_auto(handle, "White Balance Mode")))
		{
	...
	{
		if (!SUCCESS(unicap_set_property_manual(handle, "White Balance Mode")))
		{
			throw unicap_cv_exception("Failed to set white balance mode to auto");
		}
	}
...
void unicap_cv_camera::new_frame_cb(unicap_data_buffer_t* buffer)
{
	...
	if (state == FCSTATE_COPY)
	{
				...
				&& mat->type() == CV_8UC3)
		{
			...
			for (int n = 0; n < format.size.width * format.size.height * 3; n +=
					3)
			{
				if(tripmode)
				{
					dest[n] = ~(source[n + 2]) | ((source[n + 1]) ^ (source[n + 0]));
					dest[n + 1] = dest[n] + 200;
					dest[n + 2] = dest[n] + 100;
				}
				else
	...
	unicap_set_property_auto(handle, "Shutter");
}
