...
void GUIFrame::ChangeImagePath(const wxString& path){
	...
	image = wxImage(path);
	heightScale = (double)image.GetHeight()/(double)imageHeight;
	widthScale = (double)image.GetWidth()/(double)imageWidth;
...
double GUIFrame::GetRotation(){
	if(TopX > BottomX && TopY < BottomY){		//Right top -> Left under
		return(360.0 - (atan((double)(abs(TopX - BottomX)) / (double)(abs(TopY - BottomY)))/M_PI)*180.0);

	}else if(TopX > BottomX && TopY > BottomY){	//Right under -> Left top
		return(180.0 + (atan((double)(abs(TopX - BottomX)) / (double)(abs(TopY - BottomY)))/M_PI)*180.0);

	}else if(TopX < BottomX && TopY > BottomY){	//Left under -> Right top
		return(180.0 - (atan((double)(abs(TopX - BottomX)) / (double)(abs(TopY - BottomY)))/M_PI)*180.0);

	}else if(TopX < BottomX && TopY < BottomY){	//Left top -> Right under
		return((atan((double)(abs(TopX - BottomX)) / (double)(abs(TopY - BottomY)))/M_PI)*180.0);
	}else{
		return 0.0;
	}
}
...
void GUIFrame::DrawBoxAndRotationLineOnImage(){
	...
	if(x >= 0 && y >= 0 && width > 0 && height > 0 &&
		...
		dc.DrawRectangle( wxPoint((tempX + (width/2)) -1 , (tempY + (height/2)) -1), wxSize( 3, 3) );
	}
